...
function tornadomanager() {
  if (!aretornadoesactive) {
    ...
    if (ambiantsound != "playing") {
      ...
      customrepeatplay(Sound.FulgoraThunder);
      customrepeatplay(Sound.FulgoraWind);
    }
...
function repopulatetornadoes() {
  let newselec = d3.select("path" + "[daynight]")._groups[0][0];
...
function animatetornadoes(timecounter) {
  if (FSG.STATE.aretornadoesactive === true) {
    // stagger animation
    let staggerer = 5;
