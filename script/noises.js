...
function cyclerandomdistrib(param,width,height,numberofcellsatstart,averagecellarea,datamap) {
  ...
  switch (districounter) {
    ...
    case 31:
      ...
      console.log(averagecellarea,width,height,heightmin,heightmax)
      // start top left
      datamap = [];
    ...
    case 34:
      ...
      for (let i = 0; i < numberofcellsatstart / 2; i++) {
        ...
        // proceed to find the corresponding y coordinate that goes with the previous x's to place the point on a circle
    ...
    case 36:
      ...
      for (let i = 0; datamap.length < numberofcellsatstart; i++) {
        ...
        // really it's easier to picture and math remapping/linear interpolaton than rotation of coordinate i found was fun to make !
    ...
    case 37:
      ...
      xpadding = 0;
      ypadding = 0;
      // if  specified target amount of row and column 
      if (targetrowcount != 0 && targetcolumncount != 0) {
        // then number of cell on the grid is number of row * number of columns
        cellforgrid[0] = targetcolumncount * targetrowcount;
        // and we will use this computed number and not the %
        cellforgrid[1] = true;
        // calculate spacing between 2 points
        xpadding = width / targetcolumncount;
        ypadding = height / targetrowcount;
      }
      // if no specified target amount of row and column 
      else {
        // this will be number of cell we try to get but it can happen that it's too many for a nice row * column number, say 13 cell
        // in such case after the 4*3 ration is found, that 1 extra cell should be removed
        cellforgrid[0] =
          (numberofcellsatstart * percentageofcellusedforbackgroundgrid) / 100;
      }
      if (cellforgrid[1] === false) {
      ...
      // remove the extra cells that do not fit in a nice x * y grid 
      cellforgrid[0]=targetcolumncount*targetrowcount;

      // once the grid is drawn, fill in the missing points with some from circle
      for (let i = 0; i < cellforgrid[0]; i++) {
        // square grid
        let x = (xpadding / 2) + (Math.floor(i * xpadding) % width);
        let y = (ypadding / 2) + Math.floor(i / targetcolumncount) * ypadding;
      ...
      // unless the window has been resized to much taller than wide, it shouldn't crop the circle
      radius = (1 / 3) * height;
      for (let i = 0; datamap.length < numberofcellsatstart; i++) {
        // pick some x coordinate that belong to a circle of given radius and center of screen
        // this by remapping random [0:2] to [center+/-radius]
  ...
  FSG.MAIN.voronoid = FSG.MAIN.delaunayd.voronoi([0, 0, width, height]);
